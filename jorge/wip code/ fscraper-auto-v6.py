...
def getAddress(storeURL): ## it is expecting a store view!!
    print "at getAddress with:", storeURL
    global OutAdd  
    global OutCity
    global OutProv
    global OutPost
    ...
    print "##### at getAddress debugging span=", span
...
def getGeoloc(add):
    ...
    param = {'address': add, 'key': 'AIzaSyCahybZqELJlJAWmQ3p-dTRBlNtuLfHr34'} ## here used Google API
...
def getLocation(url):
    print("at getLocation with url:", url, "##")
    global countw
       
    ## Here I read the business
    ## time.sleep(1) ##justto see how behaves!!
    try:
        time.sleep(3) ## this is included to avoid the run time error reading the url!!!
        store = urllib2.urlopen(url,context = context) ## first url with category
    except Exception, error:
        print "*************************************************************************************"
        print "********* ERROR when trying to read the url. BeautifulSoup didn't like it:", error
        print "********* ERROR url to read is =", url
        print "*************************************************************************************"
        return 
        ...
        if "/view/" in a['href']:
            ...
            if counter > 0: ## delete for production!! just writing 1 for TESTING!!!! ##########################################
                ...
                try:
                    ...
                    print "################################################"
                    print "# ERROR READING URL", storeURL
                    print "################################################"
                    return
                ...
                ## It is crashing here, may be it is overloaded?
                ...
                try:
                    ...
                    print "AFTER parameters for calling google_places are lat, lng, pos =", lat, lng, pos 
                ...
                ## gets rating given a place_id
                    ...
                    ##for c in range(0,len(store.type)): ## this is for convertin format pf UTF-8 list 
                    ##    OutTyp += store.type[c]
...
print("###################### LEN of lABiz is:", len(lABiz))
...
print("###################### LEN of lASta is:", len(lASta))

for i in range(0, len(lASta)):
##for i in range(0, 3):  ## delete for production  
    lCit = getCities(lASta[i])
    lACit += lCit
    print ("%%%%%% this is the URL IM SENDING:", lASta[i], "of:", i)
    getLocation(lASta[i]) 
    

print("###################### LEN of lACit is:", len(lACit))

for i in range(0, len(lACit[i])): ## Here retrieve all stores that are present in cities
    print("%%%%%%%%%%%  CRASH?? Last for, calling getLocation with:", lACit[i])
    getLocation(lACit[i]) ## only store/view URLs need to get
